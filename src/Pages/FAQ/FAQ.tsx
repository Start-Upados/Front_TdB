  ...
  return (
    <>
      ...
      <header className="page-header">
        ...
        <p className="text-black font-semibold text-lg text-center">Encontre respostas para as dúvidas mais comuns sobre nosso trabalho</p>
      </header>
          ...
          {faqs.map(faq => (
            <div key={faq.id} className="bg-white rounded-xl border border-gray-300 shadow-sm overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-blue-600 hover:text-white transition-colors duration-200 cursor-pointer bg-transparent border-none"
              >
                <h3 className="text-primary font-semibold text-base">{faq.pergunta}</h3>
                <span className={`text-yellow-400 text-2xl font-light ml-4 shrink-0 transition-transform duration-300 ${openId === faq.id ? 'rotate-45' : ''}`}>
                  +
                </span>
              </button>
