  ...
  return (
    <>
      ...
      <div className="fixed bottom-20 right-5 z-50">
        <button
          onClick={() => setDark(p => !p)}
          className="bg-primary text-white border border-accent text-sm px-4 py-2 rounded-full cursor-pointer hover:bg-accent transition-colors duration-200"
        >
          🌓 {dark ? 'Modo Claro' : 'Modo Escuro'}
        </button>
      </div>

      <header style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 60%, #0f3460 100%)' }}
        className="px-6 py-20 md:py-28 min-h-[80vh] flex items-center">
        <div className="max-w-6xl mx-auto w-full flex flex-col md:flex-row items-center gap-10">
          ...
          <div className="flex-1 hidden md:flex justify-center">
            <img src="/img/banner-lateral-index.png" alt="Banner" className="max-h-96 object-contain" />
          </div>
        </div>
      </header>
